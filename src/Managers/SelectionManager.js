...
import { hasInvalidTransform, tempBox, tempVec } from '../Utils/utils'; // tempVec für Zentrumsberechnung
...
class SelectionManager {
    ...
    onPointerDown(event) {
        ...
        this.scene.traverseVisible((obj) => {
            ...
            if (obj === this.controlsManager.getTransformControls() ||
                ...
                obj.name === "Floor" || (this.appManager && obj === this.appManager.getCameraPivot())) { return; }
     ...
     * Wird von onPointerUp aufgerufen.
    ...
    updateSelection(targetObject, isModifierPressed) {
        // Log für Debugging (kann reduziert werden)
        ...
        // Kopie der vorherigen Auswahl für spätere UI-Updates erstellen
        const previouslySelected = [...this.selectedObjects];

        // --- Fall 1: Normaler Klick (KEINE Modifier-Taste) ---
        if (!isModifierPressed) {
            // Entferne Highlights von allen zuvor ausgewählten Objekten
            previouslySelected.forEach(obj => this.removeHighlight(obj));
            // Lösche alle gespeicherten Originalzustände für Highlights
            this.originalMaterialStates.clear();

            if (targetObject) {
                // Ein Objekt wurde direkt angeklickt
                this.selectedObjects = [targetObject]; // Setze Auswahl NUR auf dieses Objekt
                this.applyHighlight(targetObject);     // Wende Highlight an
                console.log(`[SelectionManager] Selected single: ${targetObject.name || targetObject.uuid}`);
            } else {
                // Hintergrund wurde angeklickt
                this.selectedObjects = []; // Leere die Auswahl
                console.log("[SelectionManager] Deselected all (background click)");
            }
        }
        // --- Fall 2: Klick MIT Modifier-Taste (Shift/Strg/Cmd) ---
        else {
            if (targetObject) {
                // Ein Objekt wurde mit Modifier angeklickt
                const index = this.selectedObjects.findIndex(obj => obj === targetObject);

                if (index > -1) {
                    // Objekt war BEREITS ausgewählt -> Toggle: Entferne es
                    this.removeHighlight(targetObject);          // Highlight entfernen
                    this.selectedObjects.splice(index, 1);       // Aus dem Array entfernen
                    console.log(`[SelectionManager] Deselected (modifier): ${targetObject.name || targetObject.uuid}`);
                } else {
                    // Objekt war NICHT ausgewählt -> Füge es hinzu
                    this.selectedObjects.push(targetObject);     // Zum Array hinzufügen
            ...
            // Ein Klick auf den Hintergrund MIT Modifier-Taste ändert die Auswahl nicht.
        }

        // --- Nachbereitung nach jeder Auswahländerung ---

        // 1. Aktualisiere den Gizmo (hängt ihn an Einzelobjekt oder Pivot an)
        try {
             this.updateAttachedControls();
        } catch (e) {
             console.error("[SelectionManager] Error calling updateAttachedControls:", e);
             this.controlsManager?.detach(); // Notfall-Detach bei Fehler
        }

        // 2. Aktualisiere das Highlight in der UI-Objektliste
        //    (Delegiert an UIManager -> ObjectGraphPanel)
        this.uiManager?.updateSelectionHighlight();

        // 3. Aktualisiere die Interaktivität der zugehörigen HTML-Elemente
